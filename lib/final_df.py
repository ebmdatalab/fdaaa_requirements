...
def make_row(jcs, fda_reg_dict, scrape_date=date.today()):
    """
    This is the function that processes each individual trial's information from the raw data.
    """
    ...
    try:
        ...
        trial_intervention_types = []
    td["intervention_types"] = trial_intervention_types
    ...
    td['study_first_submitted_date'] = str_to_date(variable_levels(jcs, 'study_first_submitted'))[0]
    
    td['study_submitted_date_qc'] = str_to_date(variable_levels(jcs, 'study_first_submitted_qc'))[0]
    
    td['study_first_posted_date'] = str_to_date(variable_levels(jcs, 'study_first_posted'))[0]

    td["last_updated_date"] = str_to_date(jcs['last_update_submitted'])[0]

    td['last_verified_date'] = text_or_none(jcs, ['verification_date'])

    td["certificate_date"] = str_to_date(variable_levels(jcs, 'disposition_first_submitted'))[0]

    td["certificate_date_qc"] = str_to_date(variable_levels(jcs, 'disposition_first_submitted_qc'))[0]

    td["certificate_posted_date"] = str_to_date(variable_levels(jcs, 'disposition_first_posted'))[0]
...
def make_dataframe(lines, fda_reg_dict, headers, act_filter=False, scrape_date=date.today()):
    ...
    fda_reg_dict -- points to a dict, made via the fda_reg function that contains archived data
    headers -- headers you want for your ouputted data
    act_filter -- If True, will only output covered trials under FDAAA 2007
    scrape_date -- The date you want to judge due/not due against. Default value is today's date.
    """
